...
namespace embree
{
  static int DLLLocator = 0;  // A hook to let us locate this DLL

  std::string getExecutableFileName() {
    char filename[1024];
    HMODULE hModule = NULL;
    
    // Look for the file in the same location as the DLL asking for this file
    GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                    (LPCTSTR)&DLLLocator, &hModule);
    // If that didn't work, look in the location of the executable running 
    // all this
    if (!GetModuleFileName(hModule, filename, sizeof(filename))) 
        if (!GetModuleFileName(NULL, filename, sizeof(filename))) 
            return std::string();  // Give up - return null
...
//__attribute__((constructor))
  std::string getExecutableFileName() 
  {
    ...
    // Look for the file in the same location as the DLL asking for this file 
    status = dladdr((void *)getExecutableFileName, &dl_info);
    if (status != 0)
    {
    ...
    else
    {
        ...
        bytes = readlink(pid, buf, sizeof(buf)-1);
    }
