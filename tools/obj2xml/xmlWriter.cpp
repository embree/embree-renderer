...
namespace embree
{
  ...
  void writeMaterial(FILE *file, const Ref<Material>& material) 
  {
    fprintf(file, "  <assign type=\"material\" id=\"%s\">\n",material->name.c_str());
  ...
  void writeMeshData(FILE *file, const Ref<Mesh>& mesh) 
  {
  ...
  void writeMeshHeader(FILE *file, const Ref<Mesh>& mesh, size_t meshID, size_t &offset) 
  {
